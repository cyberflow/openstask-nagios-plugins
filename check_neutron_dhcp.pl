...
sub run {
     $plugin = Nagios::Plugin->new( shortname => 'CHECK_NEUTRON_DHCP' );
...
EOT
     ...
     foreach my $a ( @agents ) {
		 if (($a->{'agent_type'} eq 'DHCP agent') && ($a->{'host'} eq $options->host)) {
            verbose (Dumper($a)."\n", 3);
		 	if ($a->{'alive'}) {
				$plugin->nagios_exit( 'OK', "DHCP agent alive" );
		 	} else {
		 		$plugin->nagios_exit( 'CRITICAL', "DHCP Agents are not be alive." );
		 	}
		 }
     }

	 $plugin->nagios_exit( 'CRITICAL', "DHCP Agents were not obtained." );     
}
