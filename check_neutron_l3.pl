...
sub run {
     $plugin = Nagios::Plugin->new( shortname => 'CHECK_NEUTRON_L3' );
...
EOT
     ...
     foreach my $a ( @agents ) {
		 if (($a->{'agent_type'} eq 'L3 agent') && ($a->{'host'} eq $options->host)) {
            verbose (Dumper($a)."\n", 3);
		 	if ($a->{'alive'}) {
				$plugin->nagios_exit( 'OK', "L3 agent alive" );
		 	} else {
		 		$plugin->nagios_exit( 'CRITICAL', "L3 agents are not be alive." );
		 	}
		 }
     }

	 $plugin->nagios_exit( 'CRITICAL', "L3 agents were not obtained." );     
}
